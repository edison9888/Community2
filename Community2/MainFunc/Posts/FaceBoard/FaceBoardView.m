...
-(id)init
{
    ...
    if (self)
    {
        ...
        _faceBoardScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, 320, 190)];
        _faceBoardScrollView.pagingEnabled = YES;
        _faceBoardScrollView.contentSize = CGSizeMake((85 / 28 + 1) * 320, 190);
        _faceBoardScrollView.showsHorizontalScrollIndicator = NO;
        _faceBoardScrollView.showsVerticalScrollIndicator = NO;
        _faceBoardScrollView.delegate = self;
        ...
        for (int i = 1; i <= 85; i++)
        {
            ...
            [_faceBoardScrollView addSubview:oneFaceButton];
            oneFaceButton = nil;
        }
        [self addSubview:_faceBoardScrollView];
...
{
    [_facePageControl setCurrentPage:_faceBoardScrollView.contentOffset.x / 320];
    [_facePageControl updateCurrentPageDisplay];
}

- (void)pageChange:(id)sender
{
    [_faceBoardScrollView setContentOffset:CGPointMake(_facePageControl.currentPage * 320, 0) animated:YES];
